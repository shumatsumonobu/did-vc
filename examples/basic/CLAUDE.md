...
- **認証アプリ** (`src/verifier/index.html`):
  ...
  - リアカメラ自動起動によるQRスキャン
  - 認証結果表示（成功・失敗）
  - 開発用テストボタン（URLパラメータ `?dev=1` で表示制御）

### 開発者モード機能
- **表示制御**: URLパラメータ `?dev=1` で開発者用テストボタンが表示
- **デフォルト非表示**: 本番環境では通常URLでアクセスするため安全
- **テスト機能**:
  - 成功テスト: 認証成功パターンの動作確認
  - 失敗テスト: 認証失敗パターンの動作確認
- **実装場所**:
  - HTML: `src/verifier/index.html` の `#dev-tools` セクション
  - JavaScript: `src/js/verifier/index.js` の URLSearchParams チェック処理
