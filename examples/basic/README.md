...
認証アプリ（Verifier）には開発・テスト用の機能が用意されています。

### 開発者モードの有効化

URLパラメータ `?dev=1` を追加することで開発者用テストボタンが表示されます：

```
通常モード: https://example.com/verifier/
開発者モード: https://example.com/verifier/?dev=1
```
...
- **デバッグ**: QRコードスキャンなしで認証フローをテスト
- **デモンストレーション**: 聴衆の前でのプレゼンテーション時
- **開発**: 新機能の動作確認

### セキュリティ

- デフォルトでは非表示（本番環境で誤って表示されない）
- URLパラメータによる明示的な有効化が必要
- 本番デプロイ時も安全
